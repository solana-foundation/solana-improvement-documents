...
input region to extract account boundary information in order to provide the
program with a slice of accounts. This parsing accounts for most of the
entrypoint's cost.
...
allows the entrypoint to derive the account slice without iterating over
...
pointers after the program ID in the program input parameters. All other
...
Each account pointer is the address of the first byte of the account
information. This is the same address that entrypoints previously had
to compute when parsing the accounts section, but it is now provided
directly as an input parameter. Duplicated accounts will have the
same pointer value, i.e., they will point to the same account.

The slice of account pointers is made available to all programs, under
all loaders, regardless of whether it is read. Since this data is
appended after the standard input parameters, existing programs are
unaffected.
...
This feature is fully backwards compatible since no program entrypoint
reads data from the input parameters after the program ID.
