...
- **Address:** `EpochStakeRoot1111111111111111111111111111` (fixed sysvar
  address, base58).
...
- SHA-256 is used for broad compatibility with light clients and cross-chain
  verifiers. For a tree of ~1-3k validators, the entire proof is only ~320
  bytes and can be verified efficiently on-chain without ZK proofs.
- The struct layout is designed to be extensible: future hash functions can be
  appended at the end without breaking existing clients that only read the
  SHA-256 root.
...
5. Compute SHA-256 leaves and Merkle root.
6. Compute `tree_lamports` as the sum of included stakes.
7. Write the `EpochStakeRootV1` struct to the ESR sysvar.

Frequency: once per epoch (~2 days).  
Cost: `O(V)` hashing for `V` validators; negligible vs existing
epoch-transition work and trivially parallelizable. With ~1-3k validators,
total computation is ~3-9ms using native SHA-256 crypto libraries.
...
- **SHA-256 hash function:** Universal compatibility with existing
  infrastructure, hardware acceleration, and broad ecosystem support. 
- **Extensible design:** The struct layout allows future hash functions (e.g.,
  BLS12-381, Poseidon for different curves) to be appended at the end without
  breaking existing clients that only read the initial SHA-256 root.
- **Direct proof verification:** With such a small tree, Merkle proofs can be
  verified directly on-chain much more cheaply than ZK proof verification, and
  even if ZK proving is desired, it would only take seconds per epoch.
...
- Future upgrades can append additional hash functions to the sysvar struct
...
- **Extensibility:** Future hash function additions can be appended to the
  sysvar struct for backward-compatible extensions.
...
- **Compute:** ~`V` leaf hashes + ~`V` inner hashes. With ~1-3k validators:
  ...
  - Easily parallelized using Rayon or similar
  - Negligible compared to overall epoch transition time
- **Storage:** One small sysvar struct (~88 bytes); negligible.
...
fn verify_validator_in_esr(esr: &EpochStakeRootV1,
    ...
    // Reconstruct leaf hash using SHA-256
